...
/**
 * Tests that a FILE_REFERENCE alias via the local (Server files) repository is
...
final class backup_alias_test extends \advanced_testcase {
    /**
     * Create a file in Course 1, add a FILE_REFERENCE alias to it
     * in Course 2's mod_resource using the local repository, then backup Course 2 with
     * MODE_GENERAL (which physically copies file bytes).
     *
     * Asserts that the backup's files/ directory contains the actual file bytes.
     */
    public function test_backup_includes_physical_bytes_for_cross_course_local_alias(): void {
        global $CFG, $USER;
     ...
     */
    public function test_restore_creates_file_from_cross_course_local_alias(): void {
        global $CFG, $DB, $USER;
        ...
        $this->assertNotEmpty($restoredfile, 'The restored resource must contain the samplefile.txt file.');
     ...
     */
    public function test_cross_site_restore_creates_file_from_local_alias(): void {
        global $CFG, $DB, $USER;
        ...
        $this->assertNotEmpty(
            $restoredfile,
            'The restored resource must contain the samplefile.txt file on cross-site restore.'
        );
    ...
    /**
     * Build the cross-course alias scenario using repository_local:
     *
     *   Course 1 - mod_resource - stored file
     *   Course 2 - mod_resource - FILE_REFERENCE alias pointing to Course 1's file via repository_local
     *
    ...
    private function setup_cross_course_alias_scenario(): array {
        ...
        // Course 1: create resource activity with a file.
        $course1 = $generator->create_course();
        $resourcecm1 = $generator->create_module('resource', ['course' => $course1->id]);
        $cmcontext1 = context_module::instance($resourcecm1->cmid);
        ...
        ], 'Sample content for local repository file');
        $contenthash = $originalfile->get_contenthash();

        // Get system instance of repository_local.
        $sql = "SELECT ri.id
                  FROM {repository} r
                  JOIN {repository_instances} ri ON ri.typeid = r.id
                 WHERE r.type = :type AND ri.contextid = :ctx";
        $repoid = $DB->get_field_sql($sql, ['type' => 'local', 'ctx' => SYSCONTEXTID]);
        $this->assertNotEmpty($repoid, 'The local repository must be installed and have a system instance.');
        ...
        // Build FILE_REFERENCE alias in Course 2's resource pointing to Course 1's file.
