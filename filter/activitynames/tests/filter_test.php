...
class filter_activitynames_filter_testcase extends advanced_testcase {
    ...
    public function test_cache() {
        ...
        format_text($html, FORMAT_HTML, array('context' => $context2));
        // Filter the text again, for course 1.
        $filtered3 = format_text($html, FORMAT_HTML, array('context' => $context1));
        // Find all the activity links in the result.
        $matches = [];
        preg_match_all('~<a class="autolink" title="([^"]*)" href="[^"]*/mod/page/view.php\?id=([0-9]+)">([^<]*)</a>~',
            $filtered3, $matches);
        // There should be 2 links now.
        $this->assertCount(2, $matches[1]);
        $this->assertEquals($page1->name, $matches[1][0]);
        $this->assertEquals($page2->name, $matches[1][1]);
    }
