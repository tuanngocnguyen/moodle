...
class text_with_options extends text {
    ...
    public function setup_form(\MoodleQuickForm $mform): void {
        ...
        $allowselection = count($options) > 0;
        if ($allowselection) {
            $elements['selector'] = $mform->createElement('autocomplete', $this->name . '_selector',
                get_string('filterfieldselector', 'core_reportbuilder', $this->get_header()), $options);
        }
    ...
    public function get_sql_filter(?array $values) : array {
        ...
        if (isset($values["{$this->name}_selector"])
            && ($operator == self::IS_EQUAL_TO || $operator == self::IS_NOT_EQUAL_TO)) {
            // Use the value of selector.
            $value = $values["{$this->name}_selector"];
            $fieldsql = $this->filter->get_field_sql();
            $params = $this->filter->get_field_params();

            switch($operator) {
                case self::IS_EQUAL_TO:
                    // We do not need case-insensitive, this will save the cost of LOWER() in sql_equal.
                    $res = $DB->sql_equal($fieldsql, ":$name", true, false);
                    $params[$name] = $value;
                    break;
                case self::IS_NOT_EQUAL_TO:
                    // We do not need case-insensitive, this will save the cost of LOWER() in sql_equal.
                    $res = $DB->sql_equal($fieldsql, ":$name", true, false, true);
                    $params[$name] = $value;
                    break;
                default:
