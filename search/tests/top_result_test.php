...
class search_top_result_testcase extends advanced_testcase {
    ...
    public function setUp() {
        ...
        // Courses.
        $this->course1 = $generator->create_course(['fullname' => 'Course 1']);
        $this->course2 = $generator->create_course(['fullname' => 'Course 2']);
     ...
     * Test course ranking
     */
    public function test_search_course_rank() {
        ...
        $this->assertEquals('Course 1', $docs[0]->get('title'));
        $this->assertEquals('Course 2', $docs[1]->get('title'));
        ...
        $this->run_index();
        $docs = $this->search->search_top($data);
        $this->assertEquals('Course 2', $docs[0]->get('title'));
        $this->assertEquals('Course 1', $docs[1]->get('title'));
    }

    /**
     * Test without teacher indexing
     */
    public function test_search_with_no_course_teacher_indexing() {
     ...
     */
    public function test_search_with_course_teacher_indexing() {
        $this->getDataGenerator()->enrol_user($this->user1->id, $this->course1->id, 'teacher');
        $this->getDataGenerator()->enrol_user($this->user2->id, $this->course1->id, 'student');
        $this->getDataGenerator()->enrol_user($this->user2->id, $this->course2->id, 'teacher');
        ...
        // Return the course and the teacher.
        $this->run_index();
        $docs = $this->search->search_top($data);
        $this->assertEquals('Course 1', $docs[0]->get('title'));
     ...
     * Test with teacher indexing
     */
    public function test_search_with_course_teacher_content_indexing() {
        ...
        $generator->create_module('forum',
            ['course' => $this->course1->id, 'name' => 'Forum 1 does not container the keyword']);
        $generator->create_module('forum',
            ['course' => $this->course2->id, 'name' => 'Forum 2 but contains keyword Course 1']);
        ...
        $this->assertEquals('Course 1', $docs[0]->get('title'));
        $this->assertEquals('User 1 Test', $docs[1]->get('title'));
        $this->assertEquals('Forum 2 but contains keyword Course 1', $docs[2]->get('title'));
    }
