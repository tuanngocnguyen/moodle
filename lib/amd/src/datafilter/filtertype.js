...
export default class {
    ...
    constructor(filterType, rootNode, initialValues) {
        ...
        this.initialValues = this.validateInitialValues(initialValues);

        this.addValueSelector();
    }

    /**
     * Validate initial values or make them usable by the filter.
     *
     * @param {Array} initialValues The initial values for the selector
     */
    ...
    async addValueSelector() {
        ...
        this.initialValues.forEach(filterValue => {
    ...
    /**
     * Get the associated condition class, which we will use later to build sql query.
     *
     * @returns {String}
     */
    get conditionclass() {
        return document.querySelector(Selectors.data.fields.byName(this.filterType)).dataset?.conditionClass;
    }
